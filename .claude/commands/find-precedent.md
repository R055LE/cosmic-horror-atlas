...
1. Restate the problem in one sentence to confirm understanding before searching.
2. Search `concepts/`, `techniques/`, `principles/`, and `authors/` for entries that touch the problem from any angle. Cast a wide net first.
3. Narrow to the most directly relevant 3-6 entries.
4. For each, report:
...
- Do not invent precedent. If the atlas doesn't cover it, say so and recommend authors who probably do.
- Prefer fewer well-fitting entries over a long list of tangential ones.
- High effort.
